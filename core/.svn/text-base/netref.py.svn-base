...
def inspect_methods(obj):
    ...
    for name in dir(obj):
        ...
        if inspect.isroutine(attr) or (hasattr(attr, "__self__") and hasattr(attr, "__call__")):
            methods[name] = inspect.getdoc(attr)
    if hasattr(obj, "__call__"):
        # darn metaclasses: dir() won't show metaclass attributes, 
        # so we must manually add __call__
        methods["__call__"] = inspect.getdoc(obj.__call__)
