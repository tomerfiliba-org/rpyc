...
def _importer(modname, *args, **kwargs):
    if modname in sys.modules:
        ...
        if isinstance(mod, RemoteModule):
            if not hasattr(router, "conn"):
                raise ImportError("No module named %s (at this splitbrain context)" % (modname,))
            # force it into the cache
            router.conn.modules[modname]
        return mod
    try:
...
if __name__ == "__main__":
    ...
    with rpyc.classic.connect("10.76.49.113") as c:
        import os
        print 1, os.getcwd()
