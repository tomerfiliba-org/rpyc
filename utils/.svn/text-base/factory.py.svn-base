...
from rpyc.utils.registry import discover_service
...
def discover(service_name, host = None, registry_ip = None, 
registry_port = None, bcast = None, timeout = 2):
    ...
    registry_ip - if given, use this ip to access the registry server
    registry_port - if given, use this ip to access the registry server
    bcast - whether or not to broadcast. if not given, broadcast will be
      used if the registry ip string contains at least one '255'
    ...
    """
    addrs = discover_service(service_name, ip = registry_ip, 
        port = registry_port, bcast = bcast, timeout = timeout)
    if not addrs:
    ...
    if not addrs:
        raise DiscoveryError("no servers exposing %r were found on %r" % (service_name, hostname))
    return addrs
