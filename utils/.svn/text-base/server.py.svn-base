...
class Server(object):
    ...
    def register(self, ip = REGISTRY_IP, reply_timeout = REGISTRY_REPLY_TIMEOUT):
        self.logger.info("registering on %s:%s", ip, REGISTRY_UDP_PORT)
    ...
    def unregister(self, ip = REGISTRY_IP):
        self.logger.info("unregistering on %s:%s", ip, REGISTRY_UDP_PORT)
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, True)
    ...
    def _bg_register(self, ip = REGISTRY_IP):
