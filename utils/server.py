...
class Server(object):
    ...
    def _accept_method(self, sock):
        ...
        been created, it should invoke _authenticate_serve_client with 'sock'
        as the argument"""
        raise NotImplementedError

    def _authenticate_serve_client(self, sock):
...
class ThreadedServer(Server):
    ...
    def _accept_method(self, sock):
        t = threading.Thread(target = self._authenticate_serve_client, args = (sock,))
        t.setDaemon(True)
        t.start()
    ...
    def _accept_method(self, sock):
        ...
        if pid == 0:
            ...
            try:
                ...
                self.listener.close()
                self.clients.clear()
                self._authenticate_serve_client(sock)
            except:
                self.logger.traceback()
            finally:
